...
/**
 * 一次获取主表单和所有的次表单的值，不作校验
 * @param form 主表单 form 对象
 * @param cases 接收一个数组，每项都是一个JEditableTable实例
 * @returns {Promise<any>}
 * @author sunjianlei
 */
export function getValuesFormAndTables(form, cases) {
  ...
  return new Promise((resolve, reject) => {
    ...
    /* form.getFieldsValue((err, values) => {
      err ? reject({ error: VALIDATE_NO_PASSED }) : resolve(values)
    }) */
  }).then(values => {
    Object.assign(options, { formValue: values })
    // 验证所有子表的表单
    return validateTables(cases, false)
  }).then(all => {
    Object.assign(options, { tablesValue: all })
    return Promise.resolve(options)
  }).catch(error => {
 ...
 * @author sunjianlei
 */
export function validateTables(cases, isvalidate) {
