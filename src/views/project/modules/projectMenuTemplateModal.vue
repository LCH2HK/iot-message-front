...
export default {
  ...
  methods: {
    loadTree () {
      var that = this
      getAction('/projectPermission/sysProjectPermission/queryTreeList').then(res => {
        if (res.success) {
    ...
    handleOk () {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          let obj
          if (!this.model.id) {
            obj = postAction('/projectPermission/sysProjectPermission/add', formData)
          } else {
            obj = putAction('/projectPermission/sysProjectPermission/edit', formData)
          }
          obj
            .then(res => {
              if (res.success) {
