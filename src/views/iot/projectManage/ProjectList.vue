    ...
    <a-col :md="12" :sm="24">
      <a-card :bordered="false">
        ...
        <a-row style="margin-left: 14px">
          ...
          <a-button @click="handleAdd(1)" type="primary">添加一级项目</a-button>
          ...
          <a-col :md="10" :sm="24">
            <template>
              <a-dropdown :trigger="[this.dropTrigger]" @visibleChange="dropStatus">
                ...
                <!--新增右键点击事件,和增加添加和删除功能-->
                <a-menu slot="overlay">
                  <a-menu-item @click="handleAdd(3)" key="1" class="right-click">
                    <img src="@assets/img/depart-manage-icon/add.png" />
                    <span>添加</span>
                  </a-menu-item>
                  <a-menu-item @click="handleDelete" key="2" class="right-click">
                    <img src="@assets/img/depart-manage-icon/delete.png" />
                    <span>删除</span>
                  </a-menu-item>
                  <a-menu-item @click="closeDrop" key="3" class="right-click">
    ...
    <a-col :md="12" :sm="24">
      <a-card :bordered="false">
        <a-form :form="form">
          ...
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="上级项目">
            <a-tree-select
              style="width:100%"
          ...
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="项目编码">
            <a-input
              disabled
              placeholder="请输入项目编码"
              v-decorator="['prjCode', validatorRules.prjCode ]"
            />
          </a-form-item>
...
import { searchByKeywords, deleteByDepartId } from '@/api/api'
import { httpAction, deleteAction, getAction } from '@/api/manage'
import { CmpListMixin } from '@/mixins/CmpListMixin'
const queryProjectTreeList = params => getAction('/sysIotProject/sysIotProject/queryTreeList', params)
// 表头
const columns = [
  {
    title: '项目名称',
...
export default {
  ...
  data () {
    return {
      ...
      validatorRules: {
        ...
        prjCode: { rules: [{ required: true, message: '请输入项目编码!' }] },
        mobile: { rules: [{ validator: this.validateMobile }] }
      },
      url: {
        delete: '/sysIotProject/sysIotProject/delete',
        edit: '/sysIotProject/sysIotProject/edit',
        deleteBatch: '/sysIotProject/sysIotProject/deleteBatch',
        exportXlsUrl: '/sysIotProject/sysIotProject/exportXls',
        importExcelUrl: '/sysIotProject/sysIotProject/importExcel'
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    // 触发onSelect事件时,为项目树右侧的form表单赋值
    setValuesToForm (record) {
      this.form.getFieldDecorator('fax', { initialValue: '' })
      this.form.setFieldsValue(
        pick(record, 'projectName', 'prjCode', 'projectOrder', 'mobile', 'fax', 'address', 'memo')
      )
    },
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          if (!this.currSelected.id) {
            this.$message.warning('请点击选择要修改项目!')
            return
          }
    ...
    handleAdd (num) {
      if (num == 1) {
        ...
        this.$refs.projectModal.add(this.rightClickSelectedKey)
        this.$refs.projectModal.title = '新增'
      }
    },
    handleDelete () {
