  ...
  <a-card :bordered="false">
      ...
      <a-form layout="inline">
        <a-row :gutter="24">
          <a-col :md="18" :sm="18">
            <a-row :gutter="24">
              <a-col :md="6" :sm="12">
                <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="姓名/账号" >
                  <a-input
                    placeholder="请输入姓名/账号查询"
              ...
              <a-col :md="6" :sm="12">
                <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="部门">
                  <j-input-lk
                    placeholder="请输入部门查询"
                    @enterSearch="enterSearch($event,'defaultOrgName')"
                    @inputValueLk="inputValueLk($event,'defaultOrgName')"
                    ref="ResetLk1"
                  ></j-input-lk>
                </a-form-item>
              </a-col>
              ...
              <template v-if="toggleSearchStatus">
                ...
                <a-col :md="6" :sm="8">
                  <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="邮箱" >
                    <j-input-lk
                      placeholder="请输入邮箱查询"
                      @enterSearch="enterSearch($event,'email')"
                      @inputValueLk="inputValueLk($event,'email')"
                      ref="ResetLk3"
                    ></j-input-lk>
                  </a-form-item>
                </a-col>
                ...
                <a-col :md="6" :sm="8">
                  <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="状态">
                    <a-select v-model="queryParam.status" placeholder="请选择用户状态查询" allowClear>
              ...
              <a-col :md="2" :sm="8">
                  ...
                  >
                    <a class="unfold-button" @click="handleToggleSearch" >
    ...
    <div class="table-operator">
      ...
      <span class="right-button">
        ...
        <a-button class="default-button" icon="download" @click="handleExportXls('用户信息')">导出</a-button>
        <a-button class="default-button" icon="download" @click="handleExportXls1('用户模板')">模板下载</a-button>
      </span>
    ...
    <div>
      ...
      >
        ...
        <template slot="attachmentAutographsolt" slot-scope="text, record">
          <div class="anty-img-wrap">
            <a-avatar
              shape="square"
              :src="getAvatarView(record.attachmentAutograph)"
              icon="contacts"
            />
          </div>
        </template>
...
import JInputLk from '@/components/cmp/JInputLk' // 模糊查询插件
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
...

        {
          ...
          customRender: (text, record, index) => {
            // 字典值替换通用方法
            if (text) {
              return filterDictText(this.sexList, text.toString())
            }
          }
        },
        {
      ...
      url: {
        ...
        ExportModule: '/sys/user/exportXlsTemplate'
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    searchReset () {
      this.queryParam = {}
      if (this.$refs.ResetLk1) {
        this.$refs.ResetLk1.ResetValue()
      }
      if (this.$refs.ResetLk2) {
        this.$refs.ResetLk2.ResetValue()
      }
      if (this.$refs.ResetLk3) {
        this.$refs.ResetLk3.ResetValue()
      }
      this.loadData(1)
    },
    handleExportXls1 (fileName) {
      ...
      downFile(this.url.ExportModule, param).then((data) => {
        if (!data) {
