...
export default {
  ...
  methods: {
    ...
    showAnnouncement (record) { // 点击消息的标题
      ...
      var that = this
      getAction('/sys/annountCement/listByUser').then((res) => { // 更新通知图标的数字
        if (res.success) {
          const topDiv = this.$parent.$parent.$parent.$parent.$children[0]
          const topMsgNumDom = that.$el.getElementsByClassName('current')[0]
          if (res.result.anntMsgTotal == '0') {
            topDiv.$el.getElementsByClassName('ant-scroll-number')[0].remove()
          } else {
    ...
    toMyAnnouncement () {
      this.$options.methods.handleHoverChange(false)
      this.$router.push({
