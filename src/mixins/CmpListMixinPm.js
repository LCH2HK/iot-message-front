...
export const CmpListMixinPm = {
  ...
  methods: {
    ...
    batchDel1 () {
      if (this.selectedRowKeys.length != 0) {
        this.batchAbandone(this.selectionRows[0].guid)
      } else {
        this.$message.warning('请选择一条记录！')
      }
    },
    ...
    handleChange1 (info, that, k, list = 'fileList') {
      if (info.file.status === 'removed') {
        if (that.attachment[k].fileTokens.indexOf(',') != -1) {
          let target = info.file.response.result.fileTokens
          let str = that.attachment[k].fileTokens
    ...
    initChartAndComment (that, record, chartUrl, taskCommentUrl) {
      if (JSON.stringify(record) !== '{}') {
        ...
        getAction(httpGetUrlTc, { guid: that.model.guid }).then((res) => {
